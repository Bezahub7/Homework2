...
```{r}
 names= paste("Patient", 1:20, sep = "_")
names(pre_bp) <- names(pre_bp) <- paste("Patient", 1:20, sep = "_")
...
treatment <- data.frame( names, pre_bp, post_bp, diff_bp)
treatment
```
...
Create a new data frame with this data in it that is similar to the data frame from task 2 (including
...
placebo <- data.frame(
  ...
  diff_bp= c(138, 135, 147, 117, 152, 134, 114, 121, 131, 130)- c(105, 136, 123, 130, 134, 143, 135, 139, 120, 124)
  , newcol=c(105, 136, 123, 130, 134, 143, 135, 139, 120, 124)>120)
placebo
```
...
```{r}
my_list <- list(treatment, placebo)
my_list
```
